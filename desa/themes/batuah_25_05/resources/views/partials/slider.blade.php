...
.grap-title-top{font-size:150%!important;text-transform:uppercase;}
.grap-title-middle{font-size:155%!important;}
.grapsvg5-title{font-size:130%!important;width:100%;text-align:center;}
.grap-title-bottom{font-size:120%!important;}
@media (max-width: 992px) {
.grap-title-bottom{font-size:100%!important;}	
}
</style>
@endpush

@php $templateslide = theme_config('templateslide', ''); @endphp
@php $slidetitle1 = theme_config('slidetitle1', ''); @endphp
@php $slidetitle2 = theme_config('slidetitle2', ''); @endphp
@php $slidetitle3 = theme_config('slidetitle3', ''); @endphp
@php $slidetitle4 = theme_config('slidetitle4', ''); @endphp

@if (theme_config('templateslide') == 'true')
	<div class="desktop-only" style="padding:10px 10px 0;">
		<div class="templateslide">
			...
			<div class="grapsvg5">
				...
				<div class="grapsvg5-title">Desa Cerdas</div>
			</div>
			<div class="grapsvg6">
			...
			<div class="grap-title flexcenter">
				...
				<div class="grap-title-top">
					@if ($slidetitle1)
						{{ $slidetitle1 }}
					@else
						Pemerintah {{ ucwords(setting('sebutan_desa')) }}
					@endif
				</div>
				<script>
				...
				<div class="grap-title-middle">
					...
					@else
						...
						<script>
							window.ityped.init(document.querySelector('.ityped'),{
							strings: ['{{ ucwords($desa['nama_desa']) }}'],
							loop: true
							})
						</script>
					@endif
				...
				<div class="grap-title-bottom">
					@if ($slidetitle3)
						{{ $slidetitle3 }}
					@else
						{{ ucwords(setting('sebutan_desa')) }} Cerdas & Berdata
					@endif
				</div>
				<div class="grap-title-bottom2">
					@if ($slidetitle4)
						{{ $slidetitle4 }}
					@else
						{{ ucwords(setting('sebutan_desa')) }} menjadi lebih efektif dan efisien dalam melakukan tugas dan fungsi dengan baik dalam melayani masyarakat
					@endif
				</div>
				</div>
			</div>
	...
	<div class="slidecustom-mobile hiddenrelative margin-top-10">
		<div class="slider-container" style="background-image: url('{{ $latar_website }}');">
			<div class="sliderstyle" style="position:relative;overflow:hidden;">
				<div style="min-height:0px;" class="bcarousel js-flickity" data-flickity='{ "autoPlay": true, "cellAlign": "left", "fade": "true" }'>
					@foreach($slider_gambar['gambar'] as $data)
						@php
						$img = $slider_gambar['lokasi'] . 'sedang_' . $data['gambar'];
		...
		<div class="grapsvg-mob-title">
			<h1>
				@if ($slidetitle2)
					{{ $slidetitle2 }}
				@else
					{{ ucwords(setting('sebutan_desa')) }} {{ ucwords($desa['nama_desa']) }}
				@endif
			</h1>
