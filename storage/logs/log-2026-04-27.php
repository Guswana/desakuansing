...
ERROR - 2026-04-27 07:19:19 --> updateConfigFile password - 
ERROR - 2026-04-27 07:19:20 --> updateConfigFile password - eyJpdiI6InJoY3A4eHpscnlrSTgwMk03YlREdXc9PSIsInZhbHVlIjoid01tdWRScktwVmVBYkFRaUN3aTNEdz09IiwibWFjIjoiMTFlMjEzYWE3NjRmMmQ5NjY4NGRhODM4MmQ0NTMxZWQ2Y2ZlOWYzNmYyMTU1YWM1OGU1MWNjOWM0MjhiMGM3YiIsInRhZyI6IiJ9
ERROR - 2026-04-27 14:19:21 --> Query error: The user specified as a definer ('perhentianluas'@'localhost') does not exist - Invalid query: SELECT `u`.*, COUNT(p.id) AS jumlah, COUNT(CASE WHEN p.sex = 1 THEN p.id END) AS laki, COUNT(CASE WHEN p.sex = 2 THEN p.id END) AS perempuan
FROM `tweb_penduduk_sex` `u`
LEFT JOIN `penduduk_hidup` `p` ON `u`.`id` = `p`.`sex` AND `p`.`config_id` = 1
LEFT JOIN `tweb_wil_clusterdesa` `a` ON `p`.`id_cluster` = `a`.`id`
GROUP BY `u`.`id`
ERROR - 2026-04-27 14:19:21 --> ["Error Number: 1449","The user specified as a definer ('perhentianluas'@'localhost') does not exist","SELECT `u`.*, COUNT(p.id) AS jumlah, COUNT(CASE WHEN p.sex = 1 THEN p.id END) AS laki, COUNT(CASE WHEN p.sex = 2 THEN p.id END) AS perempuan\nFROM `tweb_penduduk_sex` `u`\nLEFT JOIN `penduduk_hidup` `p` ON `u`.`id` = `p`.`sex` AND `p`.`config_id` = 1\nLEFT JOIN `tweb_wil_clusterdesa` `a` ON `p`.`id_cluster` = `a`.`id`\nGROUP BY `u`.`id`","Filename: C:\/laragon\/www\/desakuansing\/vendor\/codeigniter\/framework\/system\/database\/DB_driver.php","Line Number: 692"]
ERROR - 2026-04-27 14:19:57 --> Query error: The user specified as a definer ('perhentianluas'@'localhost') does not exist - Invalid query: SELECT `u`.*, COUNT(p.id) AS jumlah, COUNT(CASE WHEN p.sex = 1 THEN p.id END) AS laki, COUNT(CASE WHEN p.sex = 2 THEN p.id END) AS perempuan
FROM `tweb_penduduk_sex` `u`
LEFT JOIN `penduduk_hidup` `p` ON `u`.`id` = `p`.`sex` AND `p`.`config_id` = 1
LEFT JOIN `tweb_wil_clusterdesa` `a` ON `p`.`id_cluster` = `a`.`id`
GROUP BY `u`.`id`
ERROR - 2026-04-27 14:19:57 --> ["Error Number: 1449","The user specified as a definer ('perhentianluas'@'localhost') does not exist","SELECT `u`.*, COUNT(p.id) AS jumlah, COUNT(CASE WHEN p.sex = 1 THEN p.id END) AS laki, COUNT(CASE WHEN p.sex = 2 THEN p.id END) AS perempuan\nFROM `tweb_penduduk_sex` `u`\nLEFT JOIN `penduduk_hidup` `p` ON `u`.`id` = `p`.`sex` AND `p`.`config_id` = 1\nLEFT JOIN `tweb_wil_clusterdesa` `a` ON `p`.`id_cluster` = `a`.`id`\nGROUP BY `u`.`id`","Filename: C:\/laragon\/www\/desakuansing\/vendor\/codeigniter\/framework\/system\/database\/DB_driver.php","Line Number: 692"]
ERROR - 2026-04-27 15:17:10 --> GuzzleHttp\Exception\ConnectException: cURL error 28: Failed to connect to pantau.opensid.my.id port 443 after 4003 ms: Timeout was reached (see https://curl.haxx.se/libcurl/c/libcurl-errors.html) for https://pantau.opensid.my.id/api/track/desa?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6bnVsbCwidGltZXN0YW1wIjoxNjAzNDY2MjM5fQ.HVCNnMLokF2tgHwjQhSIYo6-2GNXB4-Kf28FSIeXnZw in C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php:210
Stack trace:
...
#20 {main}
ERROR - 2026-04-27 15:19:37 --> GuzzleHttp\Exception\ConnectException: cURL error 28: Failed to connect to pantau.opensid.my.id port 443 after 4014 ms: Timeout was reached (see https://curl.haxx.se/libcurl/c/libcurl-errors.html) for https://pantau.opensid.my.id/api/track/desa?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6bnVsbCwidGltZXN0YW1wIjoxNjAzNDY2MjM5fQ.HVCNnMLokF2tgHwjQhSIYo6-2GNXB4-Kf28FSIeXnZw in C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php:210
Stack trace:
#0 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php(158): GuzzleHttp\Handler\CurlFactory::createRejection(Object(GuzzleHttp\Handler\EasyHandle), Array)
#1 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php(110): GuzzleHttp\Handler\CurlFactory::finishError(Object(GuzzleHttp\Handler\CurlHandler), Object(GuzzleHttp\Handler\EasyHandle), Object(GuzzleHttp\Handler\CurlFactory))
#2 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlHandler.php(47): GuzzleHttp\Handler\CurlFactory::finish(Object(GuzzleHttp\Handler\CurlHandler), Object(GuzzleHttp\Handler\EasyHandle), Object(GuzzleHttp\Handler\CurlFactory))
...
#19 C:\laragon\www\desakuansing\index.php(284): require_once('C:\\laragon\\www\\...')
#20 {main}
ERROR - 2026-04-27 15:32:38 --> GuzzleHttp\Exception\ConnectException: cURL error 28: Failed to connect to pantau.opensid.my.id port 443 after 4010 ms: Timeout was reached (see https://curl.haxx.se/libcurl/c/libcurl-errors.html) for https://pantau.opensid.my.id/api/track/desa?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6bnVsbCwidGltZXN0YW1wIjoxNjAzNDY2MjM5fQ.HVCNnMLokF2tgHwjQhSIYo6-2GNXB4-Kf28FSIeXnZw in C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php:210
Stack trace:
#0 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php(158): GuzzleHttp\Handler\CurlFactory::createRejection(Object(GuzzleHttp\Handler\EasyHandle), Array)
#1 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlFactory.php(110): GuzzleHttp\Handler\CurlFactory::finishError(Object(GuzzleHttp\Handler\CurlHandler), Object(GuzzleHttp\Handler\EasyHandle), Object(GuzzleHttp\Handler\CurlFactory))
#2 C:\laragon\www\desakuansing\vendor\guzzlehttp\guzzle\src\Handler\CurlHandler.php(47): GuzzleHttp\Handler\CurlFactory::finish(Object(GuzzleHttp\Handler\CurlHandler), Object(GuzzleHttp\Handler\EasyHandle), Object(GuzzleHttp\Handler\CurlFactory))
...
#19 C:\laragon\www\desakuansing\index.php(284): require_once('C:\\laragon\\www\\...')
#20 {main}
ERROR - 2026-04-27 08:46:18 --> 404 Page Not Found: 
ERROR - 2026-04-27 08:46:29 --> 404 Page Not Found: 
ERROR - 2026-04-27 08:47:07 --> 404 Page Not Found: 
