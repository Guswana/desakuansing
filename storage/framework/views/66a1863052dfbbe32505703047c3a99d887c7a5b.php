...
<div class="table-responsive">
    <table class="table table-hover table-striped kode-isian">
        <tbody id="dragable-form-utama">
            ...
            <?php $jumlah_isian = 0; ?>
            <?php $__currentLoopData = $kode_isian; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(!$value->statis): ?>
                    <?php $jumlah_isian++; ?>
                    <tr class="duplikasi ui-sortable-handle" id="gandakan-<?php echo e($key); ?>" data-id="<?php echo e($key); ?>">
                        ...
                        <td><input type="text" name="nama_kode[]" class="form-control input-sm isian" value="<?php echo e($value->nama); ?>" placeholder="Masukkan Nama" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                        </td>
                        <td><input type="text" name="label_kode[]" class="form-control input-sm isian" value="<?php echo e($value->label ?? ''); ?>" placeholder="Masukkan Label" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                        </td>
                        <td><input type="text" name="deskripsi_kode[]" class="form-control input-sm isian" value="<?php echo e($value->deskripsi); ?>" placeholder="Masukkan Placeholder" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                        </td>
                        <td class="text-center">
                            <input class="isian-required" type="checkbox" value="1" <?= ($value->required) ? 'checked' : ''; ?> <?= ($value->tipe == '') ? 'disabled' : ''; ?> name="required_kode[<?php echo e($key); ?>]">
                        </td>
                        <td class="text-center">
                            <select class="form-control input-sm" name="kolom[]">
                                ...
                                <?php $__currentLoopData = range(1, 12); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($item); ?>" <?= ($item == $value->kolom) ? 'selected' : ''; ?>>col-<?php echo e($item); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </td>
                        <td>
                            <textarea class="form-control input-sm isian isian-atribut" name="atribut_kode[]" rows="5" placeholder="Masukkan Atribut" <?= ($value->tipe == '') ? 'disabled' : ''; ?>><?php echo e($value->atribut); ?></textarea>
                        </td>
                        <td>
                            <textarea class="form-control input-sm isian isian-pilihan <?php echo e($value->tipe == 'select-otomatis' || $value->tipe == 'select-manual' ? 'hide' : ''); ?>" name="pilihan_kode[]" rows="5" placeholder="Masukkan Pilihan"
                                <?php echo e($value->tipe != 'select-otomatis' || $value->tipe != 'select-manual' ? 'disabled' : ''); ?>

                            ><?php echo e(json_encode($value->pilihan)); ?>

                            </textarea>
                            <select class="<?php echo e($value->tipe == 'select-manual' ? 'select2' : 'hide'); ?> form-control selectinput-sm isian select-manual" name="pilihan_kode[<?php echo e($jumlah_isian); ?>][]" multiple placeholder="Masukkan Pilihan" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                                <?php $__currentLoopData = $value->pilihan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($item); ?>" selected><?php echo e($item); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                            <select class="form-control input-sm isian isian-referensi <?= ($value->tipe == 'select-otomatis') ? 'show' : 'hide'; ?>" name="referensi_kode[]" placeholder="Masukkan Pilihan" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                        ...
                        <td class="padat">
                            <div class="btn-group-vertical">
                                ...
                                <input type="hidden" class="form-control input-sm kaitkan hide" name="kaitkan_kode[]" value="<?php echo e($value->kaitkan_kode ?? ''); ?>" />
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
            ...
            <?php if($jumlah_isian == 0): ?>
                <tr class="duplikasi ui-sortable-handle" id="gandakan-0" data-id="0">
                    ...
                    <td><input type="text" name="nama_kode[]" class="form-control input-sm isian" placeholder="Masukkan Nama" <?= ($value->tipe == '') ? 'disabled' : ''; ?>></td>
                    <td><input type="text" name="label_kode[]" class="form-control input-sm isian" placeholder="Masukkan Label" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                    </td>
                    <td><input type="text" name="deskripsi_kode[]" class="form-control input-sm isian" placeholder="Masukkan Placeholder" <?= ($value->tipe == '') ? 'disabled' : ''; ?>></td>
                    <td class="text-center"><input class="isian-required" type="checkbox" value="1" <?= ($value->required) ? 'checked' : ''; ?> <?= ($value->tipe == '') ? 'disabled' : ''; ?> name="required_kode[<?php echo e($jumlah_isian); ?>]"></td>
                    <td class="text-center">
                        <select class="form-control input-sm" name="kolom[]">
                            <option value="" selected>Pilihan lebar</option>
                            <?php $__currentLoopData = range(1, 12); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($item); ?>">col-<?php echo e($item); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </td>
                    <td>
                    ...
                    <td>
                        <textarea class="form-control input-sm isian isian-pilihan  <?= ($value->tipe != 'select-manual') ? 'show' : 'hide'; ?>" name="pilihan_kode[]" rows="5" placeholder="Masukkan Pilihan" <?= ($value->tipe == '') ? 'disabled' : ''; ?>><?php echo e((string) $value->atribut); ?></textarea>
                        <select class="form-control input-sm isian select-manual <?= ($value->tipe == 'select-manual') ? 'show' : 'hide'; ?>" name="pilihan_kode[<?php echo e($jumlah_isian); ?>][]" multiple placeholder="Masukkan Pilihan" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                        </select>
                        <select class="form-control input-sm isian isian-referensi <?= ($value->tipe == 'select-otomatis') ? 'show' : 'hide'; ?>" name="referensi_kode[]" placeholder="Masukkan Pilihan" <?= ($value->tipe == '') ? 'disabled' : ''; ?>>
                            <option value="" selected>Pilihan Referensi</option>
    ...
    <script>
        $(document).ready(function() {
            ...
            function tambahKondisiIsian(elm) {
                ...
                var kategori_nama = <?php echo json_encode($kategori_nama, 15, 512) ?>;
                var kategori = <?php echo json_encode($kategori, 15, 512) ?>;
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\laragon\www\desakuansing\resources\views/admin/pengaturan_surat/kode_isian.blade.php ENDPATH**/ ?>